  ...
  def build_boost(self, attributes: set, label: typing.Any, splitting_criteria: Callable, num_trees: int,
    ...
    for i in range(num_trees):
      ...
      for index in range(m):
        epsilon_t += 1 / 2 * weights[index] * tree.__predict_single(self.__data, index) * (self.__data.iloc[index])[label]

      alpha_t: float = 1 / 2 * math.log((1 - epsilon_t) / epsilon_t)
      for index in range(m):
        weights[index] = weights[index] * np.exp(
          -alpha_t * tree.__predict_single(self.__data, index) * (self.__data.iloc[index])[label])
